...
from data_provider.uea import subsample
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.set_type == 0 and self.args.augmentation_ratio > 0:
            self.data_x, self.data_y, augmentation_tags = (
                run_augmentation_single(self.data_x, self.data_y, self.args)
            )
...
class Dataset_Custom(Dataset):
    def __init__(
        ...
        self.categorical_cols = (
            categorical_cols  # New parameter for categorical columns
        )
    ...
    def __read_data__(self):
        ...
        if self.categorical_cols is not None:
            categorical_features = [
                col for col in cols if col in self.categorical_cols
            ]
            train_data_categorical = df_raw[categorical_features][
            ...
            if self.set_type == 0:
                ...
                if not os.path.exists(path):
                    os.makedirs(path)
                joblib.dump(
                    self.encoder, os.path.join(path, "onehotencoder.pkl")
                )

            categorical_encoded = self.encoder.transform(
                df_raw[categorical_features].values
            )
            ...
            df_raw = pd.concat(
                [
                    df_raw.drop(categorical_features, axis=1),
            ...
            final_columns = (
                ["date"]
                + list(categorical_encoded_df.columns)
                + [
                    col
                    for col in df_raw.columns
                    if col not in categorical_encoded_df.columns
                    and col != "date"
                ]
            )
        ...
        if self.scale:
            ...
            train_data_continuous = df_data[non_categorical_columns][
                border1s[0] : border2s[0]
            ]
            ...
            data_continuous = self.scaler.transform(
                df_data[non_categorical_columns].values
            )
            ...
            if self.categorical_cols:
                ...
                data = pd.concat(
                    [data_continuous_df, data_categorical], axis=1
                )
            else:
    ...
    def __read_data__(self):
        ...
        if self.categorical_cols:
            if self.encoder is None:
                path = os.path.join(
                    self.checkpoints, self.setting, "onehotencoder.pkl"
                )
        ...
        if self.scale:
            ...
            if self.scaler is None:
                path = os.path.join(
                    self.checkpoints, self.setting, "scaler.pkl"
                )
...
class Dataset_M4(Dataset):
    ...
    def __getitem__(self, index):
        ...
        outsample_mask = np.zeros(
            (self.pred_len + self.label_len, 1)
        )  # m4 dataset
...
class UEAloader(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        return self.instance_norm(torch.from_numpy(batch_x)), torch.from_numpy(
            labels
        )
